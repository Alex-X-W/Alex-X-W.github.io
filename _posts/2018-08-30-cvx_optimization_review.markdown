...
Consider $\mathcal A = \{ (f(x)^T, h(x)^T, f_0(x))\, | \, x\in \mathcal D\} + \mathbf{R}_+^m \times \{0\}  \times \mathbf{R}_+$ 

and $\mathcal B = \{(u, v, t)\, | \, u=0, v = 0, t \lt p^\ast \}$.
...
- Backtrack on $||r||_2$. $r$ is the primal-dual residual $\begin{pmatrix} \nabla f(x)+A^Tb \\ Ax-b \end{pmatrix}$
  - $t \leftarrow 1$
  - while $||r(x+t\Delta x, \nu + t\Delta \nu)||_2\gt (1-\alpha t)||r(x, \nu)||_2$, do $t \leftarrow \beta t$
...
- **Log barrier**
  ...
  The **log barrier function** $\sum_i -(1/t) \log(-f_i(x))$ is an approximation to the ideal barrier $\sum_iI_-(-f_i(x))$, where the $t$ here is a parameter. The log barrieir is convex so it preserves the convexity of the objective. As $t \rightarrow \infin$, the log barrier approaches the ideal barrier.
